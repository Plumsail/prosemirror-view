...
export function initInput(view) {
  ...
  view.domObserver = new DOMObserver(view, (from, to, typeOver, added) =>
    ...
    view.dom.addEventListener(
      ...
      (view.eventHandlers[event] = (event) => {
        if (
          ...
          * This checking function causes issues with handlers.
          * For more details see the function description
...
* This checking function does not allow
* 'drop' event handlers to work. Uncomment
* if more suitable solution will be found.
  ...
  up(event) {
    ...
    if (
      !this.view.dom.contains(
        event.target.nodeType == 3 ? event.target.parentNode : event.target
      )
    )
      return;
...
function capturePaste(view, e) {
  if (!view.dom.parentNode) return;
  let plainText =
    view.shiftKey || view.state.selection.$from.parent.type.spec.code;
...
function doPaste(view, text, html, e) {
  ...
  if (view.someProp('handlePaste', (f) => f(view, e, slice || Slice.empty)))
    return true;
...
editHandlers.paste = (view, e) => {
  ...
  if (
    data &&
    doPaste(view, data.getData('text/plain'), data.getData('text/html'), e)
  )
    e.preventDefault();
...
editHandlers.drop = (view, e) => {
  ...
  if (
    view.someProp('handleDrop', (f) => f(view, e, slice || Slice.empty, move))
  ) {
    e.preventDefault();
    return;
  }
  ...
  if (!view.focused) {
    ...
    setTimeout(() => {
      if (
        view.docView &&
        view.hasFocus() &&
        !view.domObserver.currentSelection.eq(view.root.getSelection())
      )
        selectionToDOM(view);
    }, 20);
  }
